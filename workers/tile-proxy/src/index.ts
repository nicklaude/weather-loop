/**
 * Weather Tile Proxy - Cloudflare Worker
 *
 * Proxies weather tile requests with caching to avoid:
 * - CORS issues (all requests go through our domain)
 * - Rate limiting (Cloudflare edge caches tiles)
 * - 429/503 errors from upstream providers
 *
 * Supported endpoints:
 * - /rainviewer/* - RainViewer radar tiles
 * - /iem/* - Iowa Environmental Mesonet (KBOX radar)
 * - /gibs/* - NASA GIBS (GOES, VIIRS)
 */
...
const UPSTREAMS: Record<string, string> = {
  rainviewer: "https://tilecache.rainviewer.com",
...
const CACHE_TTLS: Record<string, number> = {
  rainviewer: 300, // 5 minutes - radar updates frequently
  iem: 300, // 5 minutes
  gibs: 600, // 10 minutes - GOES updates every 10 min
};
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Parse provider from path: /rainviewer/v2/radar/... -> rainviewer
    ...
    try {
      const upstreamResponse = await fetch(upstreamUrl, {
        headers: {
          "User-Agent": "WeatherLoop/1.0 (https://nicklaude.github.io/weather-loop)",
